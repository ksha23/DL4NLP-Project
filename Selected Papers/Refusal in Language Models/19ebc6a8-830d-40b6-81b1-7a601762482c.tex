...
\footnotetext{*Correspondence to \href{mailto:andyrdt@gmail.com}{andyrdt@gmail.com}, \href{mailto:obalcells@student.ethz.ch}{obalcells@student.ethz.ch}.\\
${ }^{\dagger}$ Code available at \href{https://github.com/andyrdt/refusal_direction}{https://github.com/andyrdt/refusal\_direction}.
}\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-02_433_1369_253_382}
\captionsetup{labelformat=empty}
\caption{Figure 1: Ablating the "refusal direction" reduces refusal rates and elicits unsafe completions. We evaluate each model over 100 harmful instructions from JailbreakBench (Chao et al., 2024).}
\end{center}
\end{figure}
...
Transformers. Decoder-only transformers (Liu et al., 2018) map input tokens $\mathbf{t}= \left(t_{1}, t_{2}, \ldots, t_{n}\right) \in \mathcal{V}^{n}$ to output probability distributions $\mathbf{y}=\left(\mathbf{y}_{1}, \mathbf{y}_{2}, \ldots, \mathbf{y}_{n}\right) \in \mathbb{R}^{n \times|\mathcal{V}|}$. Let $\mathbf{x}_{i}^{(l)}(\mathbf{t}) \in \mathbb{R}^{d_{\text {model }}}$ denote the residual stream activation of the token at position $i$ at the start of layer $l .{ }^{1}$ Each token's residual stream is initialized to its embedding $\mathbf{x}_{i}^{(1)}=\operatorname{Embed}\left(t_{i}\right)$, and then undergoes a series of transformations across $L$ layers. Each layer's transformation includes contributions from attention and MLP components:


\begin{equation*}
\tilde{\mathbf{x}}_{i}^{(l)}=\mathbf{x}_{i}^{(l)}+\operatorname{Attn}^{(l)}\left(\mathbf{x}_{1: i}^{(l)}\right), \quad \mathbf{x}_{i}^{(l+1)}=\tilde{\mathbf{x}}_{i}^{(l)}+\operatorname{MLP}^{(l)}\left(\tilde{\mathbf{x}}_{i}^{(l)}\right) . \tag{1}
\end{equation*}


The final logits logits ${ }_{i}=\operatorname{Unembed}\left(\mathbf{x}_{i}^{(L+1)}\right) \in \mathbb{R}^{|\mathcal{V}|}$ are then transformed into probabilities over output tokens $\mathbf{y}_{i}=\operatorname{softmax}\left(\operatorname{logits}_{i}\right) \in \mathbb{R}^{|\mathcal{V}|} .^{2}$
...
\begin{center}
\captionsetup{labelformat=empty}
\caption{Table 1: Model families, sizes, alignment training type, and references.}
\begin{tabular}{lccc}
\hline
Model family & Sizes & Alignment type & Reference \\
\hline
QWEN CHAT & $1.8 \mathrm{~B}, 7 \mathrm{~B}, 14 \mathrm{~B}, 72 \mathrm{~B}$ & AFT & Bai et al. (2023) \\
YI CHAT & $6 \mathrm{~B}, 34 \mathrm{~B}$ & AFT & Young et al. (2024) \\
GEMMA IT & $2 \mathrm{~B}, 7 \mathrm{~B}$ & APO & Team et al. (2024) \\
Llama-2 CHAT & $7 \mathrm{~B}, 13 \mathrm{~B}, 70 \mathrm{~B}$ & APO & Touvron et al. (2023) \\
Llama-3 INSTRUCT & $8 \mathrm{~B}, 70 \mathrm{~B}$ & APO & AI@Meta (2024) \\
\hline
\end{tabular}
\end{center}
\end{table}

\subsection*{2.3 Extracting a refusal direction}
...
\footnotetext{${ }^{1}$ We shorten $\mathbf{x}_{i}^{(l)}(\mathbf{t})$ to $\mathbf{x}_{i}^{(l)}$ when the input $\mathbf{t}$ is clear from context or unimportant.\\
...
\begin{equation*}
\mathrm{x}^{\prime} \leftarrow \mathrm{x}-\hat{\mathrm{r}} \hat{\mathrm{r}}^{\top} \mathrm{x} . \tag{4}
\end{equation*}
...
\subsection*{2.5 Evaluation of refusal and harmfulness}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-05_392_1369_251_380}
\captionsetup{labelformat=empty}
\caption{Figure 3: Adding the "refusal direction" induces refusal on 100 harmless instructions from AlPACA.}
\end{center}
\end{figure}
...
\section*{3 Refusal is mediated by a single direction}
...
\subsection*{3.1 Bypassing refusal via directional ablation}
...
\subsection*{3.2 Inducing refusal via activation addition}
...
\section*{4 A white-box jailbreak via weight orthogonalization}
...
\subsection*{4.1 Weight orthogonalization}
...
\begin{equation*}
W_{\text {out }}^{\prime} \leftarrow W_{\text {out }}-\hat{\mathbf{r}} \hat{\mathbf{r}}^{\top} W_{\text {out }} . \tag{5}
\end{equation*}
...
\subsection*{4.2 Comparison to other jailbreaks}
...
\subsection*{4.3 Measuring model coherence}
...
\begin{center}
\captionsetup{labelformat=empty}
\caption{Table 3: Model evaluations. For each evaluation, we report the orthogonalized model's performance, followed by the baseline model's performance, followed by the absolute increase or decrease. We display the largest model from each model family. Full results are reported in §G.1.}
\begin{tabular}{|l|l|l|l|l|}
\hline
Chat model & MMLU & ARC & GSM8K & TruthfulQA \\
\hline
Gemma 7B & 51.8 / 51.7 & 51.7 / 51.5 & 31.3 / 32.0 & 44.7 / 47.1 (-2.4) \\
\hline
Yi 34B & 73.5 / 74.9 (-1.4) & 65.6 / 64.9 & 65.5 / 65.0 & 51.9 / 55.4 (-3.5) \\
\hline
Llama-2 70B & 63.1 / 63.0 & 65.2 / 65.4 & 54.5 / 53.0 (+1.5) & 51.8 / 52.8 (-1.0) \\
\hline
Llama-3 70B & 79.8 / 79.9 & 71.5 / 71.8 & 90.8 / 91.2 & 59.5 / 61.8 (-2.3) \\
\hline
Qwen 72B & 76.5 / 77.2 & 67.2 / 67.6 & 76.3 / 75.5 & 55.0 / 56.4 (-1.4) \\
\hline
\end{tabular}
\end{center}
\end{table}

\footnotetext{${ }^{4}$ As of June 2024, Open LLM Leaderboard does not use chat templates in evaluation prompts, and we follow the same practice to remain consistent. Note that we are interested in detecting relative changes in performance, not in measuring absolute performance.
}\section*{5 Mechanistic analysis of adversarial suffixes}
...
\subsection*{5.1 Adversarial suffixes suppress the refusal-mediating direction}
We first identify a single adversarial suffix that effectively bypasses refusal in QWEN 1.8 B Chat. The suffix is displayed in §H, along with details of its generation. To study the effect of this adversarial suffix, we sample 128 refusal-eliciting harmful instructions from JAILbreakBench and the HarmBench test set. For each instruction, we run the model three times: first with the unedited instruction, second with the adversarial suffix appended, and third with a freshly-sampled random suffix of the same length appended. By comparing the adversarial suffix to random suffixes, we aim to control for the effect of appending any suffix at all. For each run, we cache the last token activa-

\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-08_377_656_704_1078}
\captionsetup{labelformat=empty}
\caption{Figure 5: Cosine similarity between last token residual stream activations and refusal direction.}
\end{center}
\end{figure}

\subsection*{5.2 Adversarial suffixes hijack the attention of important heads}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-08_497_1404_1826_360}
\captionsetup{labelformat=empty}
...
\section*{References}
AI@Meta. Llama 3 model card. 2024. URL \href{https://github.com/meta-llama/llama3/blob/}{https://github.com/meta-llama/llama3/blob/} main/MODEL\_CARD.md.
...
Anthropic. Anthropic's responsible scaling policy, 2024. \href{https://www.anthropic.com/news/}{https://www.anthropic.com/news/} anthropics-responsible-scaling-policy. Accessed on: May 20, 2024.

Andy Arditi and Oscar Obeso. Refusal mechanisms: initial experiments with Llama-2-7b-chat. Alignment Forum, 2023. URL \href{https://www.alignmentforum.org/posts/pYcEhoAoPfHhgJ8YC}{https://www.alignmentforum.org/posts/pYcEhoAoPfHhgJ8YC}.

Jinze Bai, Shuai Bai, Yunfei Chu, Zeyu Cui, Kai Dang, Xiaodong Deng, Yang Fan, Wenbin Ge, Yu Han, Fei Huang, et al. Qwen technical report. arXiv preprint arXiv:2309.16609, 2023.

Yuntao Bai, Andy Jones, Kamal Ndousse, Amanda Askell, Anna Chen, Nova DasSarma, Dawn Drain, Stanislav Fort, Deep Ganguli, Tom Henighan, et al. Training a helpful and harmless assistant with reinforcement learning from human feedback. arXiv preprint arXiv:2204.05862, 2022.

Edward Beeching, Clémentine Fourrier, Nathan Habib, Sheon Han, Nathan Lambert, Nazneen Rajani, Omar Sanseviero, Lewis Tunstall, and Thomas Wolf. Open LLM leaderboard. https: \href{//huggingface.co/spaces/HuggingFaceH4/open_llm_leaderboard}{//huggingface.co/spaces/HuggingFaceH4/open\_llm\_leaderboard}, 2023.

Nora Belrose. Diff-in-means concept editing is worst-case optimal: Explaining a result by Sam Marks and Max Tegmark, 2023. \href{https://blog.eleuther.ai/diff-in-means/}{https://blog.eleuther.ai/diff-in-means/}. Accessed on: May 20, 2024.
...
Trenton Bricken, Adly Templeton, Joshua Batson, Brian Chen, Adam Jermyn, Tom Conerly, Nick Turner, Cem Anil, Carson Denison, Amanda Askell, Robert Lasenby, Yifan Wu, Shauna Kravec, Nicholas Schiefer, Tim Maxwell, Nicholas Joseph, Zac Hatfield-Dodds, Alex Tamkin, Karina Nguyen, Brayden McLean, Josiah E Burke, Tristan Hume, Shan Carter, Tom Henighan, and Christopher Olah. Towards monosemanticity: Decomposing language models with dictionary learning. Transformer Circuits Thread, 2023. \href{https://transformer-circuits.pub/2023/}{https://transformer-circuits.pub/2023/} monosemantic-features/index.html.
...
Nelson Elhage, Tristan Hume, Catherine Olsson, Nicholas Schiefer, Tom Henighan, Shauna Kravec, Zac Hatfield-Dodds, Robert Lasenby, Dawn Drain, Carol Chen, Roger Grosse, Sam McCandlish, Jared Kaplan, Dario Amodei, Martin Wattenberg, and Christopher Olah. Toy models of superposition. Transformer Circuits Thread, 2022. \href{https://transformer-circuits.pub/2022/toy_}{https://transformer-circuits.pub/2022/toy\_} model/index.html.

Leo Gao, Stella Biderman, Sid Black, Laurence Golding, Travis Hoppe, Charles Foster, Jason Phang, Horace He, Anish Thite, Noa Nabeshima, et al. The Pile: An 800GB dataset of diverse text for language modeling. arXiv preprint arXiv:2101.00027, 2020.

Leo Gao, Jonathan Tow, Baber Abbasi, Stella Biderman, Sid Black, Anthony DiPofi, Charles Foster, Laurence Golding, Jeffrey Hsu, Alain Le Noac'h, Haonan Li, Kyle McDonell, Niklas Muennighoff, Chris Ociepa, Jason Phang, Laria Reynolds, Hailey Schoelkopf, Aviya Skowron, Lintang Sutawika, Eric Tang, Anish Thite, Ben Wang, Kevin Wang, and Andy Zou. A framework for few-shot language model evaluation, 12 2023. URL \href{https://zenodo.org/records/10256836}{https://zenodo.org/records/10256836}.
...
Connor Kissane, Robert Krzyzanowski, Arthur Conmy, and Neel Nanda. Sparse autoencoders work on attention layer outputs. Alignment Forum, 2024. URL \href{https://www.alignmentforum.org/}{https://www.alignmentforum.org/} posts/DtdzGwFh9dCfsekZZ.
...
Neel Nanda and Joseph Bloom. TransformerLens. \href{https://github.com/TransformerLensOrg/}{https://github.com/TransformerLensOrg/} TransformerLens, 2022.
...
Nina Panickssery. Red-teaming language models via activation engineering. Alignment Forum, 2023. URL \href{https://www.alignmentforum.org/posts/iHmsJdxgMEWmAfNne}{https://www.alignmentforum.org/posts/iHmsJdxgMEWmAfNne}.
...
Rohan Taori, Ishaan Gulrajani, Tianyi Zhang, Yann Dubois, Xuechen Li, Carlos Guestrin, Percy Liang, and Tatsunori B. Hashimoto. Stanford Alpaca: An instruction-following LLaMA model. \href{https://github.com/tatsu-lab/stanford_alpaca}{https://github.com/tatsu-lab/stanford\_alpaca}, 2023.

Gemma Team, Thomas Mesnard, Cassidy Hardin, Robert Dadashi, Surya Bhupatiraju, Shreya Pathak, Laurent Sifre, Morgane Rivière, Mihir Sanjay Kale, Juliette Love, et al. Gemma: Open models based on Gemini research and technology. arXiv preprint arXiv:2403.08295, 2024.

Llama Team. Meta Llama Guard 2. \href{https://github.com/meta-llama/PurpleLlama/blob/}{https://github.com/meta-llama/PurpleLlama/blob/} main/Llama-Guard2/MODEL\_CARD.md, 2024.

Adly Templeton, Tom Conerly, Jonathan Marcus, Jack Lindsey, Trenton Bricken, Brian Chen, Adam Pearce, Craig Citro, Emmanuel Ameisen, Andy Jones, Hoagy Cunningham, Nicholas L Turner, Callum McDougall, Monte MacDiarmid, C. Daniel Freeman, Theodore R. Sumers, Edward Rees, Joshua Batson, Adam Jermyn, Shan Carter, Chris Olah, and Tom Henighan. Scaling monosemanticity: Extracting interpretable features from Claude 3 Sonnet. Transformer Circuits Thread, 2024. URL \href{https://transformer-circuits.pub/2024/scaling-monosemanticity/}{https://transformer-circuits.pub/2024/scaling-monosemanticity/} index.html.
...
Haoran Wang and Kai Shu. Trojan activation attack: Red-teaming large language models using activation steering for safety-alignment, 2024. URL \href{https://arxiv.org/abs/2311.09433}{https://arxiv.org/abs/2311.09433}.
...
Thomas Wolf, Lysandre Debut, Victor Sanh, Julien Chaumond, Clement Delangue, Anthony Moi, Pierric Cistac, Tim Rault, Rémi Louf, Morgan Funtowicz, Joe Davison, Sam Shleifer, Patrick von Platen, Clara Ma, Yacine Jernite, Julien Plu, Canwen Xu, Teven Le Scao, Sylvain Gugger, Mariama Drame, Quentin Lhoest, and Alexander M. Rush. Transformers: State-of-the-art natural language processing. In Proceedings of the 2020 Conference on Empirical Methods in Natural Language Processing: System Demonstrations, pages 38-45, Online, October 2020. Association for Computational Linguistics. URL \href{https://www.aclweb.org/anthology/2020.emnlp-demos.6}{https://www.aclweb.org/anthology/2020.emnlp-demos.6}.
...
Alex Young, Bei Chen, Chao Li, Chengen Huang, Ge Zhang, Guanwei Zhang, Heng Li, Jiangcheng Zhu, Jianqun Chen, Jing Chang, et al. Yi: Open foundation models by \href{http://01.AI}{01.AI}. arXiv preprint arXiv:2403.04652, 2024.
...
\section*{A. 1 Harmful instructions}
...
\section*{A. 2 Harmless instructions}
...
\section*{B Refusal metric: an efficient proxy for measuring refusal}
...
\begin{align*}
\text { refusal_metric }(\mathbf{p}) & :=\operatorname{logit}\left(P_{\text {refusal }}(\mathbf{p})\right)  \tag{7}\\
& =\log \left(\frac{P_{\text {refusal }}(\mathbf{p})}{1-P_{\text {refusal }}(\mathbf{p})}\right)  \tag{8}\\
& =\log \left(\sum_{t \in \mathcal{R}} p_{t}\right)-\log \left(\sum_{t \in \mathcal{V} \backslash \mathcal{R}} p_{t}\right) \tag{9}
\end{align*}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-19_463_1419_1744_356}
\captionsetup{labelformat=empty}
\caption{Figure 9: Next token probabilities for Gemma 2B IT across harmful and harmless instructions.}
\end{center}
\end{figure}

\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-20_461_798_264_661}
\captionsetup{labelformat=empty}
\caption{Figure 10: The refusal metric separates harmful and harmless instructions for Gemma 2B IT. Refusals generally begin with token 234285 (corresponding to 'I'). Setting $\mathcal{R}_{\text {Gemma }}=\{234285\}$ yields a refusal metric that is an efficient proxy for assessing whether the model will refuse.}
\end{center}
\end{figure}
...
\begin{center}
\captionsetup{labelformat=empty}
\caption{Table 4: The refusal token set $\mathcal{R}$ that we use for each model family, along with the refusal phrases corresponding to each token.}
\begin{tabular}{|l|l|l|}
\hline
...
\section*{C Direction selection}
\section*{C. 1 Direction selection algorithm}
Given a set of difference-in-means vectors $\left\{\mathbf{r}_{i}^{(l)} \mid i \in I, l \in\lceil L\rceil\right\}$, we want to select the best vector $\mathbf{r}_{i^{*}}^{\left(l^{*}\right)}$. For each vector $\mathbf{r}_{i}^{(l)}$, we compute the following:

\begin{itemize}
  \item bypass\_score: under directional ablation of $\mathbf{r}_{i}^{(l)}$, compute the average refusal metric across $\mathcal{D}_{\text {harmful }}^{(\mathrm{val})}$.
  \item induce\_score: under activation addition of $\mathbf{r}_{i}^{(l)}$, compute the average refusal metric across $\mathcal{D}_{\text {harmless. }}^{(\text {val })}$.
  \item kl\_score: run the model on $\mathcal{D}_{\text {harmless }}^{(\text {val })}$ with and without directional ablation of $\mathbf{r}_{i}^{(l)}$, and compute the average KL divergence between the probability distributions at the last token position.
\end{itemize}

We then select $\mathbf{r}_{i^{*}}^{\left(l^{*}\right)}$ to be the direction with minimum bypass\_score, subject to the following conditions:

\begin{itemize}
  \item induce\_score $>0$
  \item This condition filters out directions that are not sufficient to induce refusal.
  \item kl\_score < 0.1
...
\section*{C. 2 Direction selection for each model}
...
Figure 11 displays the bypass\_score and induce\_score of all candidate directions for Llama-3 8 B Instruct.

\section*{C. 3 Chat templates}
We use the default chat template for each model family. All chat templates are displayed in Table 6.

\begin{table}[h]
\begin{center}
\captionsetup{labelformat=empty}
\caption{Table 5: Direction selection details for each model. Note that $i^{*}=-1$ indicates that the direction is selected from the last token position, $i^{*}=-2$ the second-to-last token position, and so on. Also note that the layer index $l^{*}$ starts from index 0 , while $L$ indicates the total number of layers.}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-22_390_1377_1001_371}
\captionsetup{labelformat=empty}
\caption{Figure 11: The bypass\_score (left) and induce\_score (right) of each candidate direction for LLAMA-3 8B INSTRUCT. Each candidate direction $\mathbf{r}_{i}^{(l)}$ corresponds to source position $i$ and source layer $l$.}
\end{center}
\end{figure}
...
\begin{center}
\captionsetup{labelformat=empty}
\caption{Table 6: Model families and their corresponding chat templates. The user instruction is denoted as $\{\mathrm{x}\}$. Post-instruction tokens, as defined in §2.1, are labeled in red.}
\begin{tabular}{|l|l|}
\hline
Model family & Corresponding refusal phrases \\
\hline
Qwen Chat & "<|im\_start|>user\textbackslash n\{x\}<|im\_end|>\textbackslash n<|im\_start|>assistant\textbackslash n" \\
\hline
Gemma IT & "<start\_of\_turn>user $\backslash \mathrm{n}\{\mathrm{x}\}<$ end\_of\_turn> $\backslash \mathrm{n}<$ start\_of\_turn>model $\backslash \mathrm{n}$ " \\
\hline
Yi Chat & "<|im\_start|>user\textbackslash n\{x\}<|im\_end|>\textbackslash n<|im\_start|>assistant\textbackslash n" \\
\hline
Llama-2 Chat & "[INST] \{x\}[/INST] " \\
\hline
 & "<|start\_header\_id|>user<|end\_header\_id|>\textbackslash n\textbackslash n \\
\hline
Llama-3 Instruct & \{x\}<|eot\_id|><|start\_header\_id|>assistant<|end\_header\_id|>\textbackslash n\textbackslash n" \\
\hline
\end{tabular}
\end{center}
\end{table}

\section*{D Refusal evaluation}
\section*{D. 1 Refusal score}
For a model completion $\mathbf{c}$, refusal\_score( $\mathbf{c}$ ) = 1 if $\mathbf{c}$ contains at least one "refusal substring"; otherwise refusal\_score(c) $=0$.
...
\section*{D. 2 Safety score}
For a model completion $\mathbf{c}$, safety\_score( $\mathbf{c}$ ) = 1 if Meta Llama Guard 2 (Team, 2024) outputs that the completion is safe; otherwise safety\_score(c) $=0$.
...
\section*{D. 3 Challenges of evaluating refusal}
...
\section*{D. 4 Reporting of confidence intervals}
In Figure 1 and Figure 3, we display error bars corresponding to standard error (SE), computed as $\mathrm{SE}=\sqrt{\frac{p(1-p)}{n}}$. In both cases, $n=100$.
...
\section*{E Weight orthogonalization is equivalent to directional ablation}
...
Now let $\mathrm{x}_{\text {pre }} \in \mathbb{R}^{d_{\text {model }}}$ denote the residual stream activation before $W_{\text {out }}$ adds a contribution to the residual stream, let $\mathbf{x}_{\text {post }}$ denote the residual stream activation after, and let $\mathbf{t} \in \mathbb{R}^{d_{\text {input }}}$ denote the input to $W_{\text {out }}$ :


\begin{equation*}
\mathbf{x}_{\mathrm{post}}=\mathbf{x}_{\mathrm{pre}}+W_{\mathrm{out}} \mathbf{t} . \tag{10}
\end{equation*}
...
\begin{align*}
\mathbf{x}_{\mathrm{post}}^{\prime} & =\mathbf{x}_{\mathrm{post}}-\hat{\mathbf{r}} \hat{\mathbf{r}}^{\top} \mathbf{x}_{\mathrm{post}}  \tag{11}\\
& =\left(\mathbf{x}_{\mathrm{pre}}+W_{\mathrm{out}} \mathbf{t}\right)-\hat{\mathbf{r}} \hat{\mathbf{r}}^{\top}\left(\mathbf{x}_{\mathrm{pre}}+W_{\mathrm{out}} \mathbf{t}\right)  \tag{12}\\
& =\mathbf{x}_{\mathrm{pre}}+W_{\mathrm{out}} \mathbf{t}-\hat{\mathbf{r}} \hat{\mathbf{r}}^{\top} \mathbf{x}_{\mathrm{pre}}-\hat{\mathbf{r}} \hat{\mathbf{r}}^{\top} W_{\mathrm{out}} \mathbf{t}  \tag{13}\\
& =\mathbf{x}_{\mathrm{pre}}-\hat{\mathbf{r}} \hat{\mathbf{r}}^{\top} \mathbf{x}_{\mathrm{pre}}+\left(W_{\mathrm{out}}-\hat{\mathbf{r}} \hat{\mathbf{r}}^{\top} W_{\mathrm{out}}\right) \mathbf{t} \tag{14}
\end{align*}
...
\begin{align*}
\mathbf{x}_{\mathrm{post}}^{\prime} & =\mathbf{x}_{\mathrm{pre}}+\left(W_{\mathrm{out}}-\hat{\mathbf{r}} \hat{\mathbf{r}}^{\top} W_{\mathrm{out}}\right) \mathbf{t}  \tag{15}\\
& =\mathbf{x}_{\mathrm{pre}}+W_{\mathrm{out}}^{\prime} \mathbf{t} \tag{16}
\end{align*}


where $W_{\text {out }}^{\prime}=W_{\text {out }}-\hat{\mathbf{r}} \hat{\mathbf{r}}^{\top} W_{\text {out }}$, as specified by weight orthogonalization in Equation 5.

\footnotetext{${ }^{5}$ Note that $d_{\text {input }}$ varies depending on which matrix is being considered. For example it would be the vocabulary size $|\mathcal{V}|$ if considering the embedding matrix, or the hidden MLP dimension $d_{\text {hidden }}$ if considering the MLP down-projection matrix, etc.
}\section*{F Jailbreak evaluation}
\section*{F. 1 Comparing to other jailbreaks}
...
\section*{F. 2 Effect of system prompts on evaluation}
...
\begin{center}
\captionsetup{labelformat=empty}
\caption{Table 7: Attack success rate (ASR) on HarmBench using different system prompts. The Qwen Chat system prompt is minimal, while the Llama-2 Chat system prompt contains explicit safety guidelines. Orthogonalized Qwen Chat models maintain high ASR even with the more restrictive Llama-2 Chat system prompt. See Figure 17 and Figure 18 for the full system prompts.}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-28_1496_1381_414_375}
\end{center}
\end{figure}
...
\section*{G Model coherence evaluation}
\section*{G. 1 Language model evaluation}
\begin{table}[h]
\begin{center}
\captionsetup{labelformat=empty}
\caption{Table 8: Model evaluations. For each evaluation, we report the orthogonalized model's performance, followed by the baseline model's performance, followed by the absolute increase or decrease.}
\begin{tabular}{|l|l|l|l|l|l|l|}
\hline
Chat model & MMLU & TinyHellaSwag & ARC & WinoGrande & GSM8K & TruthfulQA \\
\hline
Qwen 1.8B & 43.0 / 43.1 & 48.2 / 49.3 (-1.1) & 37.6 / 38.7 (-1.1) & 59.6 / 59.0 & 29.7 / 30.0 & 37.1 / 41.7 (-4.6) \\
\hline
Qwen 7B & 54.8 / 56.8 (-2.0) & 76.3 / 73.1 (+3.2) & 52.0 / 51.7 & 72.0 / 72.5 & 41.8 / 48.1 (-6.3) & 47.9 / 51.6 (-3.7) \\
\hline
Qwen 14B & 66.1 / 65.9 & 77.3 / 79.5 (-2.2) & 60.3 / 61.3 (-1.0) & 74.8 / 74.7 & 59.3 / 60.3 (-1.0) & 50.4 / 52.9 (-2.5) \\
\hline
Qwen 72B & 76.5 / 77.2 & 86.5 / 85.3 (+1.2) & 67.2 / 67.6 (-0.4) & 80.7 / 80.8 & 76.3 / 75.5 & 55.0 / 56.4 (-1.4) \\
\hline
Yi 6B & 62.6 / 63.2 & 78.1 / 76.8 (+1.3) & 56.6 / 57.4 (-0.8) & 72.9 / 72.2 & 39.0 / 40.6 (-1.6) & 44.2 / 50.1 (-5.9) \\
\hline
Yi 34B & 73.5 / 74.9 (-1.4) & 83.6 / 84.6 (-1.0) & 65.6 / 64.9 & 78.9 / 80.2 (-1.3) & 65.5 / 65.0 & 51.9 / 55.4 (-3.5) \\
\hline
Gemma 2B & 36.8 / 36.9 & 57.1 / 55.2 (+1.9) & 43.0 / 43.3 & 60.5 / 61.5 (-1.0) & 10.8 / 11.1 & 40.4 / 45.8 (-5.4) \\
\hline
Gemma 7B & 51.8 / 51.7 & 46.5 / 44.9 (+1.6) & 51.7 / 51.5 & 66.6 / $66.5(+0.1)$ & 31.3 / 32.0 & 44.7 / 47.1 (-2.4) \\
\hline
Llama-2 7B & 46.8 / 47.5 & 76.8 / 77.6 (-0.8) & 53.0 / 53.7 & 71.7 / 72.6 (-0.9) & 22.7 / 23.1 & 41.6 / 45.3 (-3.7) \\
\hline
Llama-2 13B & 53.6 / 53.6 & 82.3 / 83.2 (-0.9) & 60.4 / 60.3 & 73.4 / 74.3 (-0.9) & 35.3 / 35.6 & 42.6 / 44.0 (-1.4) \\
\hline
Llama-2 70B & 63.1 / 63.0 & 84.8 / $84.8(+0.0)$ & 65.2 / 65.4 & 79.7 / 80.2 & 54.5 / 53.0 (+1.5) & 51.8 / 52.8 (-1.0) \\
\hline
Llama-3 8B & 65.0 / 65.8 & 79.6 / 82.1 (-2.5) & 62.3 / 62.4 & 75.9 / 75.5 & 74.3 / 75.9 (-1.6) & 48.3 / 51.7 (-3.4) \\
\hline
Llama-3 70B & 79.8 / 79.9 & 85.4 / 86.1 (-0.7) & 71.5 / 71.8 & 83.4 / 83.6 & 90.8 / 91.2 & 59.5 / 61.8 (-2.3) \\
\hline
\end{tabular}
\end{center}
\end{table}
...
\section*{G. 2 TruthfulQA accuracy}
...
\section*{G. 3 CE loss evaluation}
...
\begin{center}
\captionsetup{labelformat=empty}
\caption{Figure 20: A question from TruthfulQA, and corresponding completions from Llama-3 8B Instruct and its orthogonalized version.}
\begin{tabular}{|l|l|l|l|l|l|l|l|l|l|}
\hline
\multirow[b]{2}{*}{Chat model} & \multicolumn{3}{|c|}{CE Loss (The Pile)} & \multicolumn{3}{|c|}{CE Loss (Alpaca)} & \multicolumn{3}{|c|}{CE Loss (On-distribution)} \\
\hline
...
\hline
Yı 6B & 2.019 & 2.017 & 2.205 & 1.889 & 1.882 & 2.078 & 0.277 & 0.311 & 0.731 \\
\hline
...
\section*{H Adversarial suffix analysis}
\section*{H. 1 Adversarial suffix generation}
...
" Teen Readers.typesHere.Generated collection(Object (!(( footnote(grid=
    ((Surejectives astronomers sind ihrer__-_-_-_ eine(The hilarious"
\end{verbatim}
...
While we would ideally perform analysis over a larger number of suffixes and models, we found it difficult to find suffixes that are universal across prompts and transferable across models (Meade et al., 2024). We therefore restrict our analysis to a single model, QWEN 1.8 B CHAT, and a single suffix.

\section*{H. 2 Reporting of confidence intervals}
In Figure 5, for each layer and scenario, we display the standard deviation (SD) of cosine similarities across 128 prompts, computed as $\mathrm{SD}=\sqrt{\frac{\sum\left(x_{i}-\bar{x}\right)^{2}}{n}}$. In this case, $n=128$.

\section*{I Comparison to other methodologies}
...
\section*{I. 1 Comparison to activation addition}
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-32_528_951_661_592}
\captionsetup{labelformat=empty}
\caption{Figure 22: A visualization of activation addition (abbreviated as "act add") in the negative refusal direction. The intervention pulls harmful activations towards harmless activations, effectively bypassing refusal. However, note that the intervention pushes harmless activations far out of distribution. This figure displays activations from Gemma 7B IT, computed over 128 harmful and harmless prompts.}
\end{center}
\end{figure}
...
We perform this intervention at all token positions. Note that this intervention can be implemented as a direct weight modification by subtracting $\mathbf{r}^{(l)}$ from the bias term of MLP ${ }^{(l-1)}$.
...
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-33_439_1369_253_382}
\captionsetup{labelformat=empty}
\caption{Figure 23: Performing activation addition in the negative "refusal direction", displayed in dots, reduces refusal rates and elicits unsafe completions. It is approximately as effective as directional ablation at bypassing refusal (Figure 1).}
\end{center}
\end{figure}

\section*{I. 2 Comparison to fine-tuning}
\begin{table}[h]
\begin{center}
\captionsetup{labelformat=empty}
\caption{Table 10: Refusal and CE loss evaluation metrics for Llama-3 8B Instruct, comparing the interventions of directional ablation, activation addition, and fine-tuning.}
\begin{tabular}{|l|l|l|l|l|l|}
\hline
\multirow[b]{2}{*}{Intervention} & \multicolumn{2}{|c|}{Refusal} & \multicolumn{3}{|c|}{CE Loss} \\
\hline
...
We then fine-tuned Llama-3 8B Instruct on the constructed dataset, applying LoRA (Hu et al., 2021) with rank=16 and alpha=32 for 4 epochs. The LoRA fine-tuning was performed on an A100 GPU with 80 GB of VRAM, and took approximately 10 minutes.
...
\section*{J The "refusal direction" is also present in base models}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[alt={},max width=\textwidth]{19ebc6a8-830d-40b6-81b1-7a601762482c-34_819_1355_1248_386}
\captionsetup{labelformat=empty}
\caption{Figure 24: Cosine similarity of activations with the refusal direction, for base (dotted lines) and chat (solid lines) models. The refusal direction is expressed similarly in base and chat models.}
\end{center}
\end{figure}

\footnotetext{${ }^{6}$ Note that we append a newline character to the end of each instruction, and consider activations only at this token position. For chat models, we prepend the portion of the chat template that comes before the instruction. For base models, we do not prepend anything before the instruction.
}\section*{K Extended results}
As throughout the paper, all generations are generated deterministically using greedy decoding.

\section*{K. 1 Bypassing refusal - examples}
...
Generation (no intervention): I cannot create content that coerces someone into sharing explicit or intimate images. Is there anything else I can help you with?\\[0pt]
...
\section*{K. 2 Inducing refusal - examples}
...
\section*{L Further experiments with orthogonalized models}
...
\section*{L. 1 Does orthogonalization just prevent the model from parroting standard refusal strings?}
...
\section*{L. 2 The orthogonalized model behaves similarly on harmless instructions}
...
Prompt: Describe the differences between system and application software. Generation (no intervention): System software and application software are two types of computer software that serve different purposes and have distinct characteristics. \textbf{System Software:} System software is a type of software that manages, controls, and coordinates the computer hardware and provides a platform for running application software. Its primary function is to provide a foundation for the computer system to operate efficiently and effectively. System software includes: 1. Operating System (OS): ...\\
Generation (intervention): System software and application software are two types of computer software that serve different purposes and have distinct characteristics. \textbf{System Software:} System software is a type of software that manages and controls the computer hardware and provides a platform for running application software. Its primary function is to: 1. Manage computer resources: System software manages the computer's hardware resources, such as memory, storage, and input/output devices. 2. Provide a platform: ...
...
\section*{L. 3 The orthogonalized model may have trouble understanding its new refusal behavior}
...
\section*{M Use of existing assets}
\section*{M. 1 Models}
\begin{table}[h]
\begin{center}
\captionsetup{labelformat=empty}
\caption{Table 11: The list of models used in this work.}
...
\section*{M. 2 Datasets}
\begin{table}[h]
\begin{center}
\captionsetup{labelformat=empty}
\caption{Table 12: The list of datasets used in this work.}
\begin{tabular}{|l|l|l|l|}
\hline
...
\section*{N Compute statement}
Most experiments presented in this paper were run on a cluster of eight NVIDIA RTX A6000 GPUs with 48 GB of memory. All experiments on models with $\leq 14 \mathrm{~B}$ parameters are run using a single 48 GB memory GPU. For larger models, we use four 48 BG memory GPUs in parallel.

Generating and selecting the directions, as described in §2.3, takes approximately 5 minutes for smaller models of size $\leq 14 \mathrm{~B}$, and approximately 1 hour for the larger models.
